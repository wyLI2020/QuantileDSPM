...
fr_phitilde_2SLS <- function(N, T, Y, Ylag1, Z, W1) {
  ...
  wrZ <- as.matrix(Z[(2*N+1):(N*T),]) # Z^{\wr}
  wrZlag1 <- as.matrix(Z[(N+1):(N*(T-1)),]) # Z^{\wr}_{-1}
  wrZlag2 <- as.matrix(Z[1:(N*(T-2)),]) # Z^{\wr}_{-2}
  wrD <- wrY - wrYlag1 # D^{\wr} = Y^{\wr} - Y^{\wr}_{-1}
  wrH1 <- wrYlag1 - wrYlag2 # the 1st section of H^{\wr}, D^{\wr}_{-1} = Y^{\wr}_{-1} - Y^{\wr}_{-2}
  wrH2 <- as.vector(fc_MatrixMultip(W1, matrix(wrH1, N, T-2))) # the 2nd section of H^{\wr}, (\iota_{T-2} \otimes W_{1}) D^{\wr}_{-1}
  wrH3 <- wrZ - wrZlag1 # the 3rd section of H^{\wr}, Z^{\wr} - Z^{\wr}_{-1}
  wrH <- cbind(wrH1, wrH2, wrH3) # H^{\wr} = (D^{\wr}_{-1}, (\iota_{T-2} \otimes W_{1}) D^{\wr}_{-1}, Z^{\wr} - Z^{\wr}_{-1})
  IV_1 <- wrYlag2 - wrYlag3 # the 1st section of instrumental variable matrix, D^{\wr}_{-2} = Y^{\wr}_{-2} - Y^{\wr}_{-3}
  IV_2 <- as.vector(fc_MatrixMultip(W1, matrix(IV_1, N, T-2))) # the 2nd section of instrumental variable matrix, (\iota_{T-2} \otimes W_{1}) D^{\wr}_{-2}
  IV_3 <- wrZlag1 - wrZlag2 # the 3rd section of instrumental variable matrix, Z^{\wr}_{-1} - Z^{\wr}_{-2}
  IV_df <- as.data.frame(cbind(IV_1, IV_2, IV_3)) # instrumental variable matrix to predict H^{\wr} (specifically, D^{\wr}_{-1}), R^{\wr} = H^{\wr}_{-1} = (D^{\wr}_{-2}, (\iota_{T-2} \otimes W_{1}) D^{\wr}_{-2}, Z^{\wr}_{-1} - Z^{\wr}_{-2})
  ...
  wrH2_pred <- as.vector(fc_MatrixMultip(W1, matrix(wrH1_pred, N, T-2))) # (\iota_{T-2} \otimes W_{1}) \widetilde{D}^{\wr}_{-1}
  wrH_pred <- cbind(wrH1_pred, wrH2_pred, wrH3) # predictor of H^{\wr}, \widetilde{H}^{\wr} = (\widetilde{D}^{\wr}_{-1}, (\iota_{T-2} \otimes W_{1}) \widetilde{D}^{\wr}_{-1}, Z^{\wr} - Z^{\wr}_{-1})
  phitilde <- fc_phitilde_2SLS(wrD, wrH, wrH_pred) # A consistent estimator of \phi = (\alpha, \beta, \gamma^{\prime})^{\prime}, \widetilde{\phi}, by the two step least square (2SLS) method
  return(phitilde)
}

fr_rhohatfo <- function(N, T, W2, secA1, secA2, Lambda1, lambda1, Vtilde, rho_ini) {
  # \widehat{\rho}_{fo}
  ...
  BhatI <- fc_B(rhohatI, N, W2) # B(\widehat{\rho}_{I})
  Sigma_varepsilontilde <- fc_Sigmavarepsilonest(BhatI, Vtilde, N, T) # \widetilde{\Sigma}_{\varepsilon} with \widetilde{\varepsilon} = S(\widehat{\rho}_{I}) \widetilde{V}
  Sigma_gtilde <- fc_Sigmagest(Sigma_varepsilontilde, secA1, secA2, N) # \widetilde{\Sigma}_{g}
...
fr_varphihat <- function(tauseq, p, X, thetahat) {
...
fr_asd_rhohatfo <- function(rhohatfo, Sigma_varepsilonhat, N, T, secA1, secA2, Lambda1) {
  # ASD of \widehat{\rho}_{fo} by theoretical results
  Sigma_ghat <- fc_Sigmagest(Sigma_varepsilonhat, secA1, secA2, N) # \widehat{\Sigma}_{g}
...
fr_asd_rhocheck <- function(Sigma_varepsiloncheck, N, T, secA1, secA2, Lambda1) {
  # ASD of \check{\rho} by theoretical results
  Sigma_gcheck <- fc_Sigmagest(Sigma_varepsiloncheck, secA1, secA2, N) # \check{\Sigma}_{g}
...
fr_varphihatB_TS <- function(pistar, tauseq, p, X, thetahatB) {
