...
//   arma::mat I_T = noZ_fc_I(T);
//   arma::mat B = noZ_fc_B(rho, N, W2);
...
arma::vec noZ_fc_Vtilde(arma::vec etatilde, int N, int T) {
  ...
  arma::mat etatilde_mat = noZ_fc_asmat(etatilde, N, T);
  arma::mat iota_T = noZ_fc_iota(T);
...
arma::mat noZ_fc_Lambda1lambda1(arma::vec Vtilde, int N, int T, arma::mat W2, arma::mat secA1, arma::mat secA2) {
  ...
  arma::mat Vtilde_mat = noZ_fc_asmat(Vtilde, N, T);
  ...
  arma::vec M_Vtilde = noZ_fc_asvec(M_Vtilde_mat); 
  arma::vec A1_Vtilde = noZ_fc_asvec(A1_Vtilde_mat); 
  arma::vec A2_Vtilde = noZ_fc_asvec(A2_Vtilde_mat); 
  arma::vec A1_M_Vtilde = noZ_fc_asvec(A1_M_Vtilde_mat); 
  arma::vec A2_M_Vtilde = noZ_fc_asvec(A2_M_Vtilde_mat); 
  arma::vec A2T_M_Vtilde = noZ_fc_asvec(A2T_M_Vtilde_mat); 
...
arma::vec noZ_fc_phihat(arma::vec etatilde, arma::mat Bhat, arma::mat Sigma_varepsilonhat, int N, int T, arma::vec Y, arma::mat Zbreve) {
  ...
  arma::mat Ybartilde_mat = noZ_fc_asmat(Ybartilde, N, T);
  ...
  arma::vec OmegahatInverse_Ybartilde = noZ_fc_asvec(OmegahatInverse_Ybartilde_mat);
  arma::mat OmegahatInverse_Zbreve(N*T, dimphi); OmegahatInverse_Zbreve.fill(0.0); // \widehat{\Omega}^{-1} \breve{Z}
  for(int l = 0; l < dimphi; l++) {
    arma::mat Zbrevel_mat = noZ_fc_asmat(Zbreve.col(l), N, T); // the l-th column of \breve{Z}
    arma::mat OmegahatInverse_Zbrevel_mat = secOmegahatInverse * Zbrevel_mat;
    arma::vec OmegahatInverse_Zbrevel = noZ_fc_asvec(OmegahatInverse_Zbrevel_mat);
...
double noZ_IQR_rcpp(arma::vec x){
  // noZ_IQR_rcpp - IQR() in R, interquartile range of the x values
  Function f("IQR");
...
double noZ_dnorm_rcpp(double x){
  // noZ_dnorm_rcpp - dnorm() in R, Gaussian kernel function
  Function f("dnorm");
  NumericVector result = f(Named("x") = x);
  return result(0);
}
...
// [[Rcpp::export]]
arma::vec noZ_fc_Xihattauseq(arma::vec tauseq, int N, arma::vec thetahat, arma::mat Xwith1, arma::mat varphihat) { // Gaussian kernel
  ...
  for (int j = 0; j < taunum; j++) {
    ...
    double Rhat = noZ_IQR_rcpp(xitauhat); // interquartile of the \widehat{Xi}(\tau)
    ...
    for (int i = 0; i < N; i++) {
      K(i) = noZ_dnorm_rcpp((0.0 - xitauhat(i)) / h);
    }
    // \widehat{\Xi}(\tau)
...
arma::mat noZ_fc_ASDvarphihat(arma::vec tauseq, int N, int p, arma::vec thetahat, arma::mat X, arma::mat varphihat){
  ...
  int taunum = tauseq.n_elem;
  int dim = 1+p; 
  arma::mat iota_N = noZ_fc_iota(N);
  arma::mat Xwith1 = join_rows(iota_N, X);
  arma::vec Xihattauseq = noZ_fc_Xihattauseq(tauseq, N, thetahat, Xwith1, varphihat); // the j-th element is \widehat{\Xi}(\tau_j)
...
arma::mat noZ_f_Ystarmat_Generation_TS(int N, int T, int q, arma::vec y0, double rhohat, arma::vec phihat, arma::vec thetahat, arma::vec varepsilonstar, arma::mat W1, arma::mat W2) {
  ...
  Ystar_mat.col(0) = y0;
  for(int t = 1; t < T+1; t++) {
    Ystar_mat.col(t) = thetahat + alphahat * Ystar_mat.col(t-1) + betahat * W1 * Ystar_mat.col(t-1) + Bhat.i() * varepsilonstar_mat.col(t-1);
  }
  return Ystar_mat;
}
...
arma::vec noZ_fc_varepsilonstar_TS(arma::vec estar, int N, int T, arma::mat Bhat, arma::vec etahat) {
  ...
  arma::vec Vhat = noZ_fc_Vtilde(etahat, N, T); // \widehat{V}, using \widehat{\eta}
  arma::mat Vhat_mat = noZ_fc_asmat(Vhat, N, T);
  arma::mat varepsilonhat_mat = Bhat * Vhat_mat;
  arma::vec varepsilonhat = noZ_fc_asvec(varepsilonhat_mat); // \widehat{\varepsilon}
